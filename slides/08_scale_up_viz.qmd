...
```{r}
mhw <- read_rds("https://github.com/jcvdav/ssf_shocks/raw/refs/heads/main/data/processed/mhw_by_turf.rds") |> 
  filter(eu_rnpa == "0203000278")

mhw$ts[[1]] |> 
  filter(between(t, ymd("2012-01-01"), ymd("2015-12-01"))) |> 
...
p2 <- ggplot(data = mhw$mhw[[1]]$event,
       mapping = aes(x = date_peak, y = intensity_max,
                     color = intensity_max)) +
  geom_linerange(mapping = aes(ymin = 0,
                               ymax = intensity_max),
