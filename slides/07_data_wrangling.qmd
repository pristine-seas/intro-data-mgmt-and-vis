...
- Dealing with missing values with `complete()`, `drop_na()`, `replace_na()`, and `fill()` (to be covered later)
:::
...
- `data_kelp` has size and abundance, but no information on allometric growth
- `fish_data` has allometric growth parameters, but no information on size and abundance
...
- Assuming data are uniquely identified
- Not handling missing values appropriately
- Not selecting all columns that need to be pivoted

. . .

**Merging:**

- Assuming column names match between datasets
- Not checking for duplicate keys
- Using wrong join type for your analysis needs
...
- Follow the three rules of tidy data

. . .

**Data Merging:**

- `left_join()`: Keep all left rows (most common)
...
- Combine tidying and merging in analysis pipelines
- Use `dplyr` verbs with tidy data for powerful analysis
